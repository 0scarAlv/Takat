...
/**
 * Earliest QUINCENA/MES payment date on or after [today]; falls back to month-end when [dayOfMonth]
 * doesn't exist in a given month. For QUINCENA, when [lastSalaryDate] shows a salary already logged
 * for the quincena that's about to start, that quincena's own end date wins over the calendar's — see
 * [FixedExpensePeriod.salaryAnchorBoundary].
 */
fun nextPaymentDate(today: LocalDate, periodType: BudgetPeriodType, dayOfMonth: Int, lastSalaryDate: LocalDate? = null): LocalDate {
    val month = YearMonth.from(today)
    return when (periodType) {
        BudgetPeriodType.QUINCENA -> {
            val fifteenth = month.atDay(15)
            val calendarBoundary = if (today <= fifteenth) fifteenth else month.atEndOfMonth()
            val anchorBoundary = lastSalaryDate?.let { FixedExpensePeriod.salaryAnchorBoundary(it) }
            if (anchorBoundary != null && anchorBoundary > calendarBoundary) anchorBoundary else calendarBoundary
        }
